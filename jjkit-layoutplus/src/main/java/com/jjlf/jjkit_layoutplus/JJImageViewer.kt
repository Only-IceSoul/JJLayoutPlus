...
import com.jjlf.jjkit_layoutplus.utils.JJAnimatorMatrixPlus
...
class JJImageViewer : AppCompatImageView {

    //region custom

     private val mMatrix = Matrix()
     private  lateinit var mGestureScale : ScaleGestureDetector
     private  lateinit var mGestureScrollDismiss : GestureDetectorCompat
     private  lateinit var mGestureScrollScale : GestureDetectorCompat
     private  lateinit var mAnimator : JJAnimatorMatrixPlus

    private fun setupViews(context:Context,attrs: AttributeSet?){
        mAnimator = JJAnimatorMatrixPlus(this)
        mGestureScale = ScaleGestureDetector(context,gestureScale())
        mGestureScrollDismiss = GestureDetectorCompat(context,gestureScroll())
        mGestureScrollScale = GestureDetectorCompat(context,gestureScrollScale())
    }
    ...
    private var mMaxScaleFactor = 5f
    fun setMaxScaleFactor(value: Float) : JJImageViewer {
        mMaxScaleFactor = value
        return this
    }

    private var mMinScaleFactor = 0.92f
    fun setMinScaleFactor(value: Float) : JJImageViewer {
        mMinScaleFactor = value
        return this
    }

    private var mListenerOnSingleTap :(()->Unit)? = null
    fun setOnSingleTapListener(listener:()-> Unit) : JJImageViewer {
        mListenerOnSingleTap = listener
        return this
    }
    private var mListenerFirstScroll :(()->Unit)? = null
    fun setOnScrollStartListener(listener:()->Unit) : JJImageViewer {
        mListenerFirstScroll = listener
        return this
    }

    private var mListenerFirstScale :(()->Unit)? = null
    fun setOnScaleStartListener(listener:()->Unit) : JJImageViewer {
        mListenerFirstScale = listener
        return this
    }

    private var mListenerRestart :(()->Unit)? = null
    fun setOnScaleRestartListener(listener :()-> Unit): JJImageViewer {
        mListenerRestart = listener
        return this
    }

    private var mListenerProgressDismiss :((Float)->Unit)? = null
    fun setOnProgressDismissListener(listener :(progress: Float)-> Unit): JJImageViewer {
        mListenerProgressDismiss = listener
        return this
    }

    private var mListenerOnDismiss :(()->Unit)? = null
    fun setOnDismissListener(listener :()-> Unit): JJImageViewer {
        mListenerOnDismiss = listener
        return this
    }
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        ...
        gestureSingleTap(event)

        if(event.pointerCount == 1 && !mIsScaling){
            mGestureScrollDismiss.onTouchEvent(event)
            invalidate()
        }
        else{
            mIsScaling = true
            mGestureScale.onTouchEvent(event)
            mGestureScrollScale.onTouchEvent(event)
            invalidate()
        }

        if(event.actionMasked == MotionEvent.ACTION_UP){

            if(mIsMovingImageForDismiss && mProgressScrollDismiss < 0.4f && mProgressScrollDismiss > -0.45f){
                mListenerRestart?.invoke()
                animateToIdentity()
                mIsMovingImageForDismiss = false
            }else if(mIsMovingImageForDismiss){
                mIsMovingImageForDismiss = false
                mListenerOnDismiss?.invoke()
            }

            updateImagePoints(imageMatrix)
            if(mIsScaling && getMapPointsWidth(mImageScalePoints) <= mScaleIdentity){
                mIsScaling = false
                mListenerRestart?.invoke()
                animateToIdentity()
            }
    ...
    private var mCounterScaleCall = 0
    private fun gestureScale() = object: ScaleGestureDetector.SimpleOnScaleGestureListener() {

        override fun onScale(detector: ScaleGestureDetector?): Boolean {
            if(mCounterScaleCall < 1){
                mListenerFirstScale?.invoke()
            }
            mCounterScaleCall++
            updateImagePoints(imageMatrix)
            val scaleFactor = (detector?.scaleFactor ?: 1f)
            val currentScale = getMapPointsWidth(mImageScalePoints)
            val minScale = mScaleIdentity * mMinScaleFactor
            val maxScale = mScaleIdentity * mMaxScaleFactor
            val scale =  if(currentScale >= maxScale && scaleFactor > 1f) 1f else if (currentScale <= minScale && scaleFactor < 1f) 1f else scaleFactor
            imageMatrix.postScale(scale,scale,detector!!.focusX,detector.focusY)
            computeTranslationFitXY()
            return true
        }
    }
    private fun gestureScrollScale() = object : GestureDetector.SimpleOnGestureListener(){
        override fun onScroll(e1: MotionEvent?, e2: MotionEvent?, distanceX: Float, distanceY: Float): Boolean {
            ...
            val dstX = if(isBitmapWidthOverLimit()) computeScrollAxisXLimitToBitmap(distanceX) else 0f
            imageMatrix.postTranslate(dstX,dstY)
            return true
        }
    }


    private var mCounterScroll = 0
    private var mIsMovingImageForDismiss = false
    private var mProgressScrollDismiss = 0f
    private fun gestureScroll() = object: GestureDetector.SimpleOnGestureListener() {
        override fun onScroll(e1: MotionEvent?, e2: MotionEvent?, distanceX: Float, distanceY: Float): Boolean {
                if (mCounterScroll < 1) {
                    mListenerFirstScroll?.invoke()
                }
                mIsMovingImageForDismiss = true
                imageMatrix.postTranslate(0f, -distanceY)
                mProgressScrollDismiss += distanceY / (height / 2f)
                mListenerProgressDismiss?.invoke(abs(mProgressScrollDismiss))
                mCounterScroll++
    ...
    private var mTapCounter = 0
    private var mRecordTimeTap = 0L
    private var mLastDistanceXTap = 0f
    private var mLastDistanceYTap = 0f
    private var mDistanceXTap = 0f
    private var mDistanceYTap = 0f
    private fun gestureSingleTap(e:MotionEvent){

        when(e.actionMasked){
            MotionEvent.ACTION_DOWN -> {
                mRecordTimeTap = SystemClock.elapsedRealtime()
                mLastDistanceXTap = e.rawX
                mLastDistanceYTap = e.rawY
                ++mTapCounter
            }
            MotionEvent.ACTION_MOVE -> {
                if(e.pointerCount == 1){
                    mDistanceXTap += e.rawX - mLastDistanceXTap
                    mDistanceYTap += e.rawY - mLastDistanceYTap
                    mLastDistanceXTap = e.rawX
                    mLastDistanceYTap = e.rawY
                }
            }
            MotionEvent.ACTION_POINTER_DOWN -> ++mTapCounter
            MotionEvent.ACTION_UP -> {
                val resultTime = SystemClock.elapsedRealtime() - mRecordTimeTap
                if(mTapCounter == 1 &&  mRecordTimeTap > 0 && resultTime < 400
                    && mDistanceXTap < 40 && mDistanceYTap < 40){
                    mListenerOnSingleTap?.invoke()
                }
                mTapCounter = 0
                mRecordTimeTap = 0L
                mDistanceXTap = 0f
                mDistanceYTap = 0f
                mLastDistanceXTap = 0f
                mLastDistanceYTap = 0f
            }
        }
    }
    ...
    //region Compute
    private var mImageRectPoints = floatArrayOf(0f,0f,0f,0f,0f,0f,0f,0f)
    private var mImageScalePoints = floatArrayOf(0f,0f,0f,0f,0f,0f,0f,0f)
    private fun updateImagePoints(matrix:Matrix){
        val bitmap = (drawable as? BitmapDrawable)?.bitmap
        bitmap?.let {
            mImageRectPoints[0] = 0f
            mImageRectPoints[1] = 0f
            mImageRectPoints[2] = it.width.toFloat()
            mImageRectPoints[3] = 0f
            mImageRectPoints[4] = it.width.toFloat()
            mImageRectPoints[5] = it.height.toFloat()
            mImageRectPoints[6] = 0f
            mImageRectPoints[7] = it.height.toFloat()
            matrix.mapPoints(mImageRectPoints)
            mImageScalePoints[0] = 0f
            mImageScalePoints[1] = 0f
            mImageScalePoints[2] = 1f
            mImageScalePoints[3] = 0f
            mImageScalePoints[4] = 1f
            mImageScalePoints[5] = 1f
            mImageScalePoints[6] = 0f
            mImageScalePoints[7] = 1f
            matrix.mapPoints(mImageScalePoints)

        }
    }


    private fun computeScrollAxisYLimitToBitmap(distance:Float) : Float {
        updateImagePoints(imageMatrix)
        val topP = getMapPointTop(mImageRectPoints)
        val bottomP = getMapPointBottom(mImageRectPoints)
        val isDragDown = distance < 0f
        return if(isDragDown){
            if(topP < 0f){
                if((topP - distance) > 0f )
                    -topP
                else -distance
            ...
            if(bottomP > height) {
                if((bottomP - distance) < height )
                    -(bottomP - height)
                else -distance
            } else 0f
        }
    }
    private fun computeScrollAxisXLimitToBitmap(distance:Float) : Float {
        updateImagePoints(imageMatrix)
        val leftP = getMapPointLeft(mImageRectPoints)
        val rightP = getMapPointRight(mImageRectPoints)
        val orientation = distance < 0f
        return if(orientation) {
            if(leftP < 0f){
                if((leftP - distance) > 0f )
                    -leftP
                else -distance
            } else 0f
        } else if(rightP > width) {
            if((rightP - distance) < width )
                -(rightP - width)
            else -distance
        } else 0f

    }
    private fun computeTranslationFitXY(){
        updateImagePoints(imageMatrix)
        val h = getMapPointsHeight(mImageRectPoints)
        val w = getMapPointsWidth(mImageRectPoints)
        val l = getMapPointLeft(mImageRectPoints)
        val t = getMapPointTop(mImageRectPoints)
        val r = getMapPointRight(mImageRectPoints)
        val b = getMapPointBottom(mImageRectPoints)
        var deltaX = 0f
        var deltaY = 0f
        when {
            h <= height -> { deltaY = (height - h) / 2 - t }
            t > 0.0F    -> { deltaY = -t }
            b <  height -> { deltaY = height - b }
        }
        when {
            w <= width -> { deltaX = (width - w) / 2 - l }
            l > 0      -> { deltaX = -l }
            r < width  -> { deltaX = width - r }
        }
        imageMatrix.postTranslate(deltaX,deltaY)
    }
    private fun isBitmapHeightOverLimit(): Boolean{
        updateImagePoints(imageMatrix)
        return getMapPointsHeight(mImageRectPoints) > height
    }
    private fun isBitmapWidthOverLimit():Boolean{
        updateImagePoints(imageMatrix)
        return getMapPointsWidth(mImageRectPoints) > width
    }
    ...
    private fun fitXY(){
        ...
        bitmap?.let {
            ...
            mScaleIdentity = min(width / getMapPointsWidth(mImageRectPoints), height / getMapPointsHeight(mImageRectPoints))
            mMatrix.postScale(mScaleIdentity,mScaleIdentity,getMapPointCenterX(mImageRectPoints),getMapPointCenterY(mImageRectPoints))
            updateImagePoints(mMatrix)
            imageMatrix = mMatrix
        }
    }
    private fun animateToIdentity(){
        val bitmap = (drawable as? BitmapDrawable)?.bitmap
        bitmap?.let {
            mMatrix.reset()
            mMatrix.postTranslate((width - bitmap.width) / 2f,(height - bitmap.height) / 2f )
    ...
    //region fun map points Rect

    private fun getMapPointsWidth(points : FloatArray) : Float {
        return getMapPointRight(points) - getMapPointLeft(points)
    }

    private fun getMapPointsHeight(points : FloatArray) : Float {
        return getMapPointBottom(points) - getMapPointTop(points)
    }

    private fun getMapPointLeft(points: FloatArray): Float {
        return min(min(min(points[0], points[2]), points[4]), points[6])
    }
    private fun getMapPointRight(points: FloatArray): Float {
        return max(max(max(points[0], points[2]), points[4]), points[6])
    }

    private fun getMapPointTop(points: FloatArray): Float {
        return min(min(min(points[1], points[3]), points[5]), points[7])
    }
    private fun getMapPointBottom(points: FloatArray): Float {
        return max(max(max(points[1], points[3]), points[5]), points[7])
    }

    private fun getMapPointCenterX(points: FloatArray): Float {
        return (getMapPointRight(points) + getMapPointLeft(points)) / 2f
    }
    private fun getMapPointCenterY(points: FloatArray): Float {
        return (getMapPointBottom(points) + getMapPointTop(points)) / 2f
    }
    ...
    fun lpWidth(w: Int) : JJImageViewer {
        mlpWidth = w
        return this
    }
    fun lpHeight(h: Int) : JJImageViewer {
        mlpHeight = h
        return this
    }
    fun lpPadding(pad: JJPadding) : JJImageViewer {
        mlpPadding = pad
        return this
    }

    fun lpMargin(mar: JJMargin) : JJImageViewer {
        mlpMargins = mar
        return this
    }

    fun lpApply() :  JJImageViewer {
        layoutParams.height = mlpHeight
        layoutParams.width = mlpWidth
        val margin = layoutParams as? ViewGroup.MarginLayoutParams
        margin?.topMargin = mlpMargins.top
    ...
    fun lplWidth(w: Int) : JJImageViewer {
        mlsWidth = w
        return this
    }
    fun lplHeight(h: Int) : JJImageViewer {
        mlsHeight = h
        return this
    }
    fun lplPadding(pad: JJPadding) : JJImageViewer {
        mlsPadding = pad
        return this
    }

    fun lplMargin(mar: JJMargin) : JJImageViewer {
        mlsMargins = mar
        return this
    }

    fun lplApply():JJImageViewer{
        layoutParams.height = mlsHeight
        layoutParams.width = mlsWidth
        val margin = layoutParams as? ViewGroup.MarginLayoutParams
        margin?.topMargin = mlsMargins.top
    ...
    fun colGravity(gravity: Int): JJImageViewer {
        setupCol()
    ...
    fun ablScrollFlags(flags: Int) : JJImageViewer {
        setupAblp()
        (layoutParams as? AppBarLayout.LayoutParams)?.scrollFlags = flags
        return this
    }

    fun ablScrollInterpolator(interpolator: Interpolator) : JJImageViewer {
        setupAblp()
    ...
    fun rlAlignParentTop(value : Boolean = true): JJImageViewer {
        setupRlp()
        val data = if(value) 1 else 0
    ...
    fun rlAlignParentStart(value : Boolean = true): JJImageViewer {
        setupRlp()
        val data = if(value) 1 else 0
    ...
    fun rlAlignParentEnd(value : Boolean = true): JJImageViewer {
        setupRlp()
        val data = if(value) 1 else 0
    ...
    fun rlAlignParentLeft(value : Boolean = true): JJImageViewer {
        setupRlp()
        val data = if(value) 1 else 0
    ...
    fun rlAlignParentRight(value : Boolean = true): JJImageViewer {
        setupRlp()
        val data = if(value) 1 else 0
    ...
    fun rlCenterInParentVertically(value:Boolean = true): JJImageViewer {
        setupRlp()
        val data = if(value) 1 else 0
    ...
    fun rlCenterInParentHorizontally(value:Boolean = true): JJImageViewer {
        setupRlp()
        val data = if(value) 1 else 0
    ...
    fun rlAlignBaseline(viewId: Int): JJImageViewer {
        setupRlp()
    ...
    fun llWeight(w: Float): JJImageViewer {
        setupLlp()
        (layoutParams as? LinearLayout.LayoutParams)?.weight = w
        return this
    }
    fun llGravity(gravity: Int): JJImageViewer {
        setupLlp()
    ...
    fun mlVisibilityMode(visibility: Int): JJImageViewer {
        mMotionConstraintSet?.setVisibilityMode(id, visibility)
        return this
    }

    fun mlVerticalBias(float: Float): JJImageViewer {
        mMotionConstraintSet?.setVerticalBias(id,float)
        return this
    }
    fun mlHorizontalBias(float: Float): JJImageViewer {
        mMotionConstraintSet?.setHorizontalBias(id,float)
        return this
    }

    fun mlCenterHorizontallyOf(viewId: Int, marginStart: Int = 0, marginEnd: Int = 0): JJImageViewer {
    ...
    fun mlCenterVerticallyOf(viewId: Int,marginTop: Int = 0, marginBottom: Int = 0): JJImageViewer {
    ...
    fun mlMargins(margins: JJMargin) : JJImageViewer {
    ...
    fun mlFloatCustomAttribute(attrName: String, value: Float): JJImageViewer {
        mMotionConstraintSet?.setFloatValue(id,attrName,value)
        return this
    }

    fun mlIntCustomAttribute(attrName: String, value: Int): JJImageViewer {
        mMotionConstraintSet?.setIntValue(id,attrName,value)
        return this
    }

    fun mlColorCustomAttribute(attrName: String, value: Int): JJImageViewer {
        mMotionConstraintSet?.setColorValue(id,attrName,value)
        return this
    }

    fun mlStringCustomAttribute(attrName: String, value: String): JJImageViewer {
        mMotionConstraintSet?.setStringValue(id,attrName,value)
        return this
    }

    fun mlRotation(float: Float): JJImageViewer {
        mMotionConstraintSet?.setRotation(id,float)
        return this
    }

    fun mlRotationX(float: Float): JJImageViewer {
        mMotionConstraintSet?.setRotationX(id,float)
        return this
    }

    fun mlRotationY(float: Float): JJImageViewer {
        mMotionConstraintSet?.setRotationY(id,float)
        return this
    }

    fun mlTranslation(x: Float,y: Float): JJImageViewer {
        mMotionConstraintSet?.setTranslation(id,x,y)
        return this
    }
    fun mlTranslationX(x: Float): JJImageViewer {
        mMotionConstraintSet?.setTranslationX(id,x)
        return this
    }

    fun mlTranslationY(y: Float): JJImageViewer {
        mMotionConstraintSet?.setTranslationY(id,y)
        return this
    }

    fun mlTranslationZ(z: Float): JJImageViewer {
        mMotionConstraintSet?.setTranslationZ(id,z)
        return this
    }

    fun mlTransformPivot(x: Float, y: Float): JJImageViewer {
        mMotionConstraintSet?.setTransformPivot(id,x,y)
        return this
    }

    fun mlTransformPivotX(x: Float): JJImageViewer {
        mMotionConstraintSet?.setTransformPivotX(id,x)
        return this
    }

    fun mlTransformPivotY(y: Float): JJImageViewer {
        mMotionConstraintSet?.setTransformPivotY(id,y)
        return this
    }

    fun mlScaleX(x: Float): JJImageViewer {
        mMotionConstraintSet?.setScaleX(id,x)
        return this
    }

    fun mlScaleY(y: Float): JJImageViewer {
        mMotionConstraintSet?.setScaleY(id,y)
        return this
    }

    fun mlDimensionRatio(ratio: String): JJImageViewer {
        mMotionConstraintSet?.setDimensionRatio(id,ratio)
        return this
    }

    fun mlAlpha(alpha: Float): JJImageViewer {
    ...
    fun mlCenterInParent(verticalBias: Float, horizontalBias: Float, margin: JJMargin): JJImageViewer {
    ...
    fun mlCenterInParentVertically(): JJImageViewer {
        mMotionConstraintSet?.connect(id, ConstraintSet.TOP, ConstraintSet.PARENT_ID, ConstraintSet.TOP, 0)
        mMotionConstraintSet?.connect(id, ConstraintSet.BOTTOM, ConstraintSet.PARENT_ID, ConstraintSet.BOTTOM, 0)
        mMotionConstraintSet?.setVerticalBias(id, 0.5f)
    ...
    fun mlCenterInParentHorizontally(): JJImageViewer {
        mMotionConstraintSet?.connect(id, ConstraintSet.START, ConstraintSet.PARENT_ID, ConstraintSet.START, 0)
        mMotionConstraintSet?.connect(id, ConstraintSet.END, ConstraintSet.PARENT_ID, ConstraintSet.END, 0)
        mMotionConstraintSet?.setHorizontalBias(id, 0.5f)
        return this
    ...
    fun mlCenterInParentVertically(bias: Float, topMargin: Int, bottomMargin: Int): JJImageViewer {
    ...
    fun mlCenterInParentHorizontally(bias: Float, startMargin: Int, endtMargin: Int): JJImageViewer {
    ...
    fun mlCenterInParentTopVertically(): JJImageViewer {
        mMotionConstraintSet?.connect(id, ConstraintSet.TOP, ConstraintSet.PARENT_ID, ConstraintSet.TOP, 0)
    ...
    fun mlCenterInParentBottomVertically(): JJImageViewer {
    ...
    fun mlCenterInParentStartHorizontally(): JJImageViewer {
        mMotionConstraintSet?.connect(id, ConstraintSet.START, ConstraintSet.PARENT_ID, ConstraintSet.START, 0)
    ...
    fun mlCenterInParentEndHorizontally(): JJImageViewer {
    ...
    fun mlCenterInTopVerticallyOf(viewId: Int): JJImageViewer {
    ...
    fun mlCenterInBottomVerticallyOf(viewId: Int): JJImageViewer {
    ...
    fun mlCenterInStartHorizontallyOf(viewId: Int): JJImageViewer {
    ...
    fun mlCenterInEndHorizontallyOf(viewId: Int): JJImageViewer {
    ...
    fun mlCenterVertically(topId: Int, topSide: Int, topMargin: Int, bottomId: Int, bottomSide: Int, bottomMargin: Int, bias: Float): JJImageViewer {
        mMotionConstraintSet?.centerVertically(id, topId, topSide, topMargin, bottomId, bottomSide, bottomMargin, bias)
        return this
    }

    fun mlCenterHorizontally(startId: Int, startSide: Int, startMargin: Int, endId: Int, endSide: Int, endMargin: Int, bias: Float): JJImageViewer {
        mMotionConstraintSet?.centerHorizontally(id, startId, startSide, startMargin, endId, endSide, endMargin, bias)
        return this
    }


    fun mlFillParent(): JJImageViewer {
    ...
    fun mlFillParent(margin: JJMargin): JJImageViewer {
        mMotionConstraintSet?.constrainWidth(id,0)
        mMotionConstraintSet?.constrainHeight(id,0)
        mMotionConstraintSet?.connect(id, ConstraintSet.TOP, ConstraintSet.PARENT_ID, ConstraintSet.TOP, margin.top)
        mMotionConstraintSet?.connect(id, ConstraintSet.START, ConstraintSet.PARENT_ID, ConstraintSet.START, margin.left)
    ...
    fun mlFillParentHorizontally(): JJImageViewer {
        mMotionConstraintSet?.constrainWidth(id,0)
        mMotionConstraintSet?.connect(id, ConstraintSet.START, ConstraintSet.PARENT_ID, ConstraintSet.START, 0)
        mMotionConstraintSet?.connect(id, ConstraintSet.END, ConstraintSet.PARENT_ID, ConstraintSet.END, 0)
        return this
    ...
    fun mlFillParentVertically(): JJImageViewer {
        mMotionConstraintSet?.constrainHeight(id,0)
        mMotionConstraintSet?.connect(id, ConstraintSet.TOP, ConstraintSet.PARENT_ID, ConstraintSet.TOP, 0)
        mMotionConstraintSet?.connect(id, ConstraintSet.BOTTOM, ConstraintSet.PARENT_ID, ConstraintSet.BOTTOM, 0)
        return this
    ...
    fun mlFillParentHorizontally(startMargin: Int, endMargin: Int): JJImageViewer {
        mMotionConstraintSet?.constrainWidth(id,0)
        mMotionConstraintSet?.connect(id, ConstraintSet.START, ConstraintSet.PARENT_ID, ConstraintSet.START, startMargin)
    ...
    fun mlFillParentVertically(topMargin: Int, bottomMargin: Int): JJImageViewer {
        mMotionConstraintSet?.constrainHeight(id,0)
        mMotionConstraintSet?.connect(id, ConstraintSet.TOP, ConstraintSet.PARENT_ID, ConstraintSet.TOP, topMargin)
        mMotionConstraintSet?.connect(id, ConstraintSet.BOTTOM, ConstraintSet.PARENT_ID, ConstraintSet.BOTTOM, bottomMargin)
        return this
    ...
    fun mlVisibility(visibility: Int): JJImageViewer {
        mMotionConstraintSet?.setVisibility(id, visibility)
        return this
    }

    fun mlElevation(elevation: Float): JJImageViewer {
        mMotionConstraintSet?.setElevation(id, elevation)
        return this
    }

    fun mlApply(): JJImageViewer {
        mMotionConstraintSet?.applyTo(parent as ConstraintLayout)
        return this
    }

    fun mlSetConstraint(cs : ConstraintSet?): JJImageViewer {
        mMotionConstraintSet = cs
        return this
    }

    fun mlDisposeConstraint(): JJImageViewer {
    ...
    fun clFloatCustomAttribute(attrName: String, value: Float): JJImageViewer {
        mConstraintSet.setFloatValue(id,attrName,value)
        return this
    }

    fun clIntCustomAttribute(attrName: String, value: Int): JJImageViewer {
        mConstraintSet.setIntValue(id,attrName,value)
        return this
    }

    fun clColorCustomAttribute(attrName: String, value: Int): JJImageViewer {
        mConstraintSet.setColorValue(id,attrName,value)
        return this
    }

    fun clStringCustomAttribute(attrName: String, value: String): JJImageViewer {
        mConstraintSet.setStringValue(id,attrName,value)
        return this
    }

    fun clRotation(float: Float): JJImageViewer {
        mConstraintSet.setRotation(id,float)
        return this
    }

    fun clRotationX(float: Float): JJImageViewer {
        mConstraintSet.setRotationX(id,float)
        return this
    }

    fun clRotationY(float: Float): JJImageViewer {
        mConstraintSet.setRotationY(id,float)
        return this
    }

    fun clTranslation(x: Float,y: Float): JJImageViewer {
        mConstraintSet.setTranslation(id,x,y)
        return this
    }
    fun clTranslationX(x: Float): JJImageViewer {
        mConstraintSet.setTranslationX(id,x)
        return this
    }

    fun clTranslationY(y: Float): JJImageViewer {
        mConstraintSet.setTranslationY(id,y)
        return this
    }

    fun clTranslationZ(z: Float): JJImageViewer {
        mConstraintSet.setTranslationZ(id,z)
        return this
    }

    fun clTransformPivot(x: Float, y: Float): JJImageViewer {
        mConstraintSet.setTransformPivot(id,x,y)
        return this
    }

    fun clTransformPivotX(x: Float): JJImageViewer {
        mConstraintSet.setTransformPivotX(id,x)
        return this
    }

    fun clTransformPivotY(y: Float): JJImageViewer {
        mConstraintSet.setTransformPivotY(id,y)
        return this
    }

    fun clScaleX(x: Float): JJImageViewer {
        mConstraintSet.setScaleX(id,x)
        return this
    }

    fun clScaleY(y: Float): JJImageViewer {
        mConstraintSet.setScaleY(id,y)
        return this
    }

    fun clDimensionRatio(ratio: String): JJImageViewer {
        mConstraintSet.setDimensionRatio(id,ratio)
        return this
    }

    fun clAlpha(alpha: Float): JJImageViewer {
        mConstraintSet.setAlpha(id,alpha)
        return this
    }


    fun clApply(): JJImageViewer {
        mConstraintSet.applyTo(parent as ConstraintLayout)
        return this
    }

    fun clVisibilityMode(visibility: Int): JJImageViewer {
        mConstraintSet.setVisibilityMode(id, visibility)
        return this
    }

    fun clVerticalBias(float: Float): JJImageViewer {
        mConstraintSet.setVerticalBias(id,float)
        return this
    }
    fun clHorizontalBias(float: Float): JJImageViewer {
        mConstraintSet.setHorizontalBias(id,float)
        return this
    }

    fun clCenterHorizontallyOf(viewId: Int, marginStart: Int = 0, marginEnd: Int = 0): JJImageViewer {
    ...
    fun clCenterVerticallyOf(viewId: Int,marginTop: Int = 0, marginBottom: Int = 0): JJImageViewer {
    ...
    fun clMargins(margins: JJMargin) : JJImageViewer {
    ...
    fun clCenterInParent(verticalBias: Float, horizontalBias: Float, margin: JJMargin): JJImageViewer {
    ...
    fun clCenterInParentVertically(): JJImageViewer {
        mConstraintSet.connect(id, ConstraintSet.TOP, ConstraintSet.PARENT_ID, ConstraintSet.TOP, 0)
        mConstraintSet.connect(id, ConstraintSet.BOTTOM, ConstraintSet.PARENT_ID, ConstraintSet.BOTTOM, 0)
        mConstraintSet.setVerticalBias(id, 0.5f)
        return this
    ...
    fun clCenterInParentHorizontally(): JJImageViewer {
        mConstraintSet.connect(id, ConstraintSet.START, ConstraintSet.PARENT_ID, ConstraintSet.START, 0)
        mConstraintSet.connect(id, ConstraintSet.END, ConstraintSet.PARENT_ID, ConstraintSet.END, 0)
        mConstraintSet.setHorizontalBias(id, 0.5f)
        return this
    ...
    fun clCenterInParentVertically(bias: Float, topMargin: Int, bottomMargin: Int): JJImageViewer {
    ...
    fun clCenterInParentHorizontally(bias: Float, startMargin: Int, endtMargin: Int): JJImageViewer {
    ...
    fun clCenterInParentTopVertically(): JJImageViewer {
        mConstraintSet.connect(id, ConstraintSet.TOP, ConstraintSet.PARENT_ID, ConstraintSet.TOP, 0)
    ...
    fun clCenterInParentBottomVertically(): JJImageViewer {
    ...
    fun clCenterInParentStartHorizontally(): JJImageViewer {
        mConstraintSet.connect(id, ConstraintSet.START, ConstraintSet.PARENT_ID, ConstraintSet.START, 0)
    ...
    fun clCenterInParentEndHorizontally(): JJImageViewer {
    ...
    fun clCenterInTopVertically(topId: Int): JJImageViewer {
    ...
    fun clCenterInBottomVertically(bottomId: Int): JJImageViewer {
    ...
    fun clCenterInStartHorizontally(startId: Int): JJImageViewer {
    ...
    fun clCenterInEndHorizontally(endId: Int): JJImageViewer {
    ...
    fun clCenterVertically(topId: Int, topSide: Int, topMargin: Int, bottomId: Int, bottomSide: Int, bottomMargin: Int, bias: Float): JJImageViewer {
        mConstraintSet.centerVertically(id, topId, topSide, topMargin, bottomId, bottomSide, bottomMargin, bias)
        return this
    }

    fun clCenterHorizontally(startId: Int, startSide: Int, startMargin: Int, endId: Int, endSide: Int, endMargin: Int, bias: Float): JJImageViewer {
        mConstraintSet.centerHorizontally(id, startId, startSide, startMargin, endId, endSide, endMargin, bias)
        return this
    }


    fun clFillParent(): JJImageViewer {
        mConstraintSet.constrainWidth(id,0)
        mConstraintSet.constrainHeight(id,0)
        mConstraintSet.connect(id, ConstraintSet.TOP, ConstraintSet.PARENT_ID, ConstraintSet.TOP, 0)
        mConstraintSet.connect(id, ConstraintSet.START, ConstraintSet.PARENT_ID, ConstraintSet.START, 0)
    ...
    fun clFillParent(margin: JJMargin): JJImageViewer {
        mConstraintSet.constrainWidth(id,0)
        mConstraintSet.constrainHeight(id,0)
        mConstraintSet.connect(id, ConstraintSet.TOP, ConstraintSet.PARENT_ID, ConstraintSet.TOP, margin.top)
        mConstraintSet.connect(id, ConstraintSet.START, ConstraintSet.PARENT_ID, ConstraintSet.START, margin.left)
    ...
    fun clFillParentHorizontally(): JJImageViewer {
        mConstraintSet.constrainWidth(id,0)
        mConstraintSet.connect(id, ConstraintSet.START, ConstraintSet.PARENT_ID, ConstraintSet.START, 0)
        mConstraintSet.connect(id, ConstraintSet.END, ConstraintSet.PARENT_ID, ConstraintSet.END, 0)
        return this
    ...
    fun clFillParentVertically(): JJImageViewer {
        mConstraintSet.constrainHeight(id,0)
        mConstraintSet.connect(id, ConstraintSet.TOP, ConstraintSet.PARENT_ID, ConstraintSet.TOP, 0)
        mConstraintSet.connect(id, ConstraintSet.BOTTOM, ConstraintSet.PARENT_ID, ConstraintSet.BOTTOM, 0)
        return this
    ...
    fun clFillParentHorizontally(startMargin: Int, endMargin: Int): JJImageViewer {
        mConstraintSet.constrainWidth(id,0)
        mConstraintSet.connect(id, ConstraintSet.START, ConstraintSet.PARENT_ID, ConstraintSet.START, startMargin)
    ...
    fun clFillParentVertically(topMargin: Int, bottomMargin: Int): JJImageViewer {
        mConstraintSet.constrainHeight(id,0)
        mConstraintSet.connect(id, ConstraintSet.TOP, ConstraintSet.PARENT_ID, ConstraintSet.TOP, topMargin)
        mConstraintSet.connect(id, ConstraintSet.BOTTOM, ConstraintSet.PARENT_ID, ConstraintSet.BOTTOM, bottomMargin)
        return this
    ...
    fun clVisibility(visibility: Int): JJImageViewer {
    ...
    fun clElevation(elevation: Float): JJImageViewer {
    ...
    fun clMinWidth(w:Int): JJImageViewer {
        mConstraintSet.constrainMinWidth(id,w)
        return this
    }

    fun clMinHeight(h:Int): JJImageViewer {
        mConstraintSet.constrainMinHeight(id,h)
        return this
    }

    fun clMaxWidth(w:Int): JJImageViewer {
        mConstraintSet.constrainMaxWidth(id,w)
        return this
    }

    fun clMaxHeight(h:Int): JJImageViewer {
    ...
    fun cllApply(): JJImageViewer {
        mConstraintSetLandScape.applyTo(parent as ConstraintLayout)
        return this
    }


    fun cllFloatCustomAttribute(attrName: String, value: Float): JJImageViewer {
        mConstraintSet.setFloatValue(id,attrName,value)
        return this
    }

    fun cllIntCustomAttribute(attrName: String, value: Int): JJImageViewer {
        mConstraintSet.setIntValue(id,attrName,value)
        return this
    }

    fun cllColorCustomAttribute(attrName: String, value: Int): JJImageViewer {
        mConstraintSet.setColorValue(id,attrName,value)
        return this
    }

    fun cllStringCustomAttribute(attrName: String, value: String): JJImageViewer {
        mConstraintSet.setStringValue(id,attrName,value)
        return this
    }

    fun cllRotation(float: Float): JJImageViewer {
        mConstraintSet.setRotation(id,float)
        return this
    }

    fun cllRotationX(float: Float): JJImageViewer {
        mConstraintSet.setRotationX(id,float)
        return this
    }

    fun cllRotationY(float: Float): JJImageViewer {
        mConstraintSet.setRotationY(id,float)
        return this
    }

    fun cllTranslation(x: Float,y: Float): JJImageViewer {
        mConstraintSet.setTranslation(id,x,y)
        return this
    }
    fun cllTranslationX(x: Float): JJImageViewer {
        mConstraintSet.setTranslationX(id,x)
        return this
    }

    fun cllTranslationY(y: Float): JJImageViewer {
        mConstraintSet.setTranslationY(id,y)
        return this
    }

    fun cllTranslationZ(z: Float): JJImageViewer {
        mConstraintSet.setTranslationZ(id,z)
        return this
    }

    fun cllTransformPivot(x: Float, y: Float): JJImageViewer {
        mConstraintSet.setTransformPivot(id,x,y)
        return this
    }

    fun cllTransformPivotX(x: Float): JJImageViewer {
        mConstraintSet.setTransformPivotX(id,x)
        return this
    }

    fun cllTransformPivotY(y: Float): JJImageViewer {
        mConstraintSet.setTransformPivotY(id,y)
        return this
    }

    fun cllScaleX(x: Float): JJImageViewer {
        mConstraintSet.setScaleX(id,x)
        return this
    }

    fun cllScaleY(y: Float): JJImageViewer {
        mConstraintSet.setScaleY(id,y)
        return this
    }

    fun cllDimensionRatio(ratio: String): JJImageViewer {
        mConstraintSet.setDimensionRatio(id,ratio)
        return this
    }

    fun cllAlpha(alpha: Float): JJImageViewer {
        mConstraintSet.setAlpha(id,alpha)
        return this
    }


    fun cllVisibilityMode(visibility: Int): JJImageViewer {
        mConstraintSetLandScape.setVisibilityMode(id, visibility)
        return this
    }

    fun cllVerticalBias(float: Float): JJImageViewer {
        mConstraintSetLandScape.setVerticalBias(id,float)
        return this
    }
    fun cllHorizontalBias(float: Float): JJImageViewer {
        mConstraintSetLandScape.setHorizontalBias(id,float)
        return this
    }

    fun cllCenterHorizontallyOf(viewId: Int, marginStart: Int = 0, marginEnd: Int = 0): JJImageViewer {
    ...
    fun cllCenterVerticallyOf(viewId: Int,marginTop: Int = 0, marginBottom: Int = 0): JJImageViewer {
    ...
    fun cllMargins(margins: JJMargin) : JJImageViewer {
    ...
    fun cllCenterInParent(verticalBias: Float, horizontalBias: Float, margin: JJMargin): JJImageViewer {
    ...
    fun cllCenterInParentVertically(): JJImageViewer {
        mConstraintSetLandScape.connect(id, ConstraintSet.TOP, ConstraintSet.PARENT_ID, ConstraintSet.TOP, 0)
        mConstraintSetLandScape.connect(id, ConstraintSet.BOTTOM, ConstraintSet.PARENT_ID, ConstraintSet.BOTTOM, 0)
        mConstraintSetLandScape.setVerticalBias(id, 0.5f)
        return this
    ...
    fun cllCenterInParentHorizontally(): JJImageViewer {
        mConstraintSetLandScape.connect(id, ConstraintSet.START, ConstraintSet.PARENT_ID, ConstraintSet.START, 0)
        mConstraintSetLandScape.connect(id, ConstraintSet.END, ConstraintSet.PARENT_ID, ConstraintSet.END, 0)
        mConstraintSetLandScape.setHorizontalBias(id, 0.5f)
        return this
    ...
    fun cllCenterInParentVertically(bias: Float, topMargin: Int, bottomMargin: Int): JJImageViewer {
    ...
    fun cllCenterInParentHorizontally(bias: Float, startMargin: Int, endtMargin: Int): JJImageViewer {
    ...
    fun cllCenterInParentTopVertically(): JJImageViewer {
        mConstraintSetLandScape.connect(id, ConstraintSet.TOP, ConstraintSet.PARENT_ID, ConstraintSet.TOP, 0)
    ...
    fun cllCenterInParentBottomVertically(): JJImageViewer {
    ...
    fun cllCenterInParentStartHorizontally(): JJImageViewer {
        mConstraintSetLandScape.connect(id, ConstraintSet.START, ConstraintSet.PARENT_ID, ConstraintSet.START, 0)
    ...
    fun cllCenterInParentEndHorizontally(): JJImageViewer {
    ...
    fun cllCenterInTopVertically(topId: Int): JJImageViewer {
    ...
    fun cllCenterInBottomVertically(bottomId: Int): JJImageViewer {
    ...
    fun cllCenterInStartHorizontally(startId: Int): JJImageViewer {
    ...
    fun cllCenterInEndHorizontally(endId: Int): JJImageViewer {
    ...
    fun cllCenterVertically(topId: Int, topSide: Int, topMargin: Int, bottomId: Int, bottomSide: Int, bottomMargin: Int, bias: Float): JJImageViewer {
        mConstraintSetLandScape.centerVertically(id, topId, topSide, topMargin, bottomId, bottomSide, bottomMargin, bias)
        return this
    }

    fun cllCenterHorizontally(startId: Int, startSide: Int, startMargin: Int, endId: Int, endSide: Int, endMargin: Int, bias: Float): JJImageViewer {
        mConstraintSetLandScape.centerHorizontally(id, startId, startSide, startMargin, endId, endSide, endMargin, bias)
        return this
    }


    fun cllFillParent(): JJImageViewer {
        mConstraintSetLandScape.constrainWidth(id,0)
        mConstraintSetLandScape.constrainHeight(id,0)
        mConstraintSetLandScape.connect(id, ConstraintSet.TOP, ConstraintSet.PARENT_ID, ConstraintSet.TOP, 0)
        mConstraintSetLandScape.connect(id, ConstraintSet.START, ConstraintSet.PARENT_ID, ConstraintSet.START, 0)
    ...
    fun cllFillParent(margin: JJMargin): JJImageViewer {
        mConstraintSetLandScape.constrainWidth(id,0)
        mConstraintSetLandScape.constrainHeight(id,0)
        mConstraintSetLandScape.connect(id, ConstraintSet.TOP, ConstraintSet.PARENT_ID, ConstraintSet.TOP, margin.top)
        mConstraintSetLandScape.connect(id, ConstraintSet.START, ConstraintSet.PARENT_ID, ConstraintSet.START, margin.left)
    ...
    fun cllFillParentHorizontally(): JJImageViewer {
        mConstraintSetLandScape.constrainWidth(id,0)
        mConstraintSetLandScape.connect(id, ConstraintSet.START, ConstraintSet.PARENT_ID, ConstraintSet.START, 0)
        mConstraintSetLandScape.connect(id, ConstraintSet.END, ConstraintSet.PARENT_ID, ConstraintSet.END, 0)
        return this
    ...
    fun cllFillParentVertically(): JJImageViewer {
        mConstraintSetLandScape.constrainHeight(id,0)
        mConstraintSetLandScape.connect(id, ConstraintSet.TOP, ConstraintSet.PARENT_ID, ConstraintSet.TOP, 0)
        mConstraintSetLandScape.connect(id, ConstraintSet.BOTTOM, ConstraintSet.PARENT_ID, ConstraintSet.BOTTOM, 0)
        return this
    ...
    fun cllFillParentHorizontally(startMargin: Int, endMargin: Int): JJImageViewer {
        mConstraintSetLandScape.constrainWidth(id,0)
        mConstraintSetLandScape.connect(id, ConstraintSet.START, ConstraintSet.PARENT_ID, ConstraintSet.START, startMargin)
    ...
    fun cllFillParentVertically(topMargin: Int, bottomMargin: Int): JJImageViewer {
        mConstraintSetLandScape.constrainHeight(id,0)
        mConstraintSetLandScape.connect(id, ConstraintSet.TOP, ConstraintSet.PARENT_ID, ConstraintSet.TOP, topMargin)
        mConstraintSetLandScape.connect(id, ConstraintSet.BOTTOM, ConstraintSet.PARENT_ID, ConstraintSet.BOTTOM, bottomMargin)
        return this
    ...
    fun cllVisibility(visibility: Int): JJImageViewer {
    ...
    fun cllElevation(elevation: Float): JJImageViewer {
    ...
    fun cllMinWidth(w:Int): JJImageViewer {
        mConstraintSetLandScape.constrainMinWidth(id,w)
        return this
    }

    fun cllMinHeight(h:Int): JJImageViewer {
        mConstraintSetLandScape.constrainMinHeight(id,h)
        return this
    }

    fun cllMaxWidth(w:Int): JJImageViewer {
        mConstraintSetLandScape.constrainMaxWidth(id,w)
        return this
    }

    fun cllMaxHeight(h:Int): JJImageViewer {
